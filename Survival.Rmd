...
```{r}
pdcds1 %>% select(is.factor) %>% summary() %>% kable()
pdcds1 %>% select(-is.factor) %>% summary() %>% kable()
```
...
out of all the samples, most of them underwent percutaneous method

```{r, results='asis'}
pdcds1_withna <- pdcds1 %>%
  replace_with_na(replace = list(RemovalDur = "365",
                                 CxDur = "365"))
method_sum <- list("Method" = list("Percutaneous Method" = ~ n_perc0(.data$Method == "Percutaneous Method"),
                                 "Peritoneoscopy Method" = ~ n_perc0(.data$Method != "Percutaneous Method")),
                   "Complication" = list("Yes" = ~ n_perc0(.data$Compx == "Yes"),
                                         "No" = ~ n_perc0(.data$Compx != "Yes")),
                   "Mean Complication Time" = list("Mean (SD)" = ~ mean_sd(.data$CxDur, na_rm = T, show_n = "never", denote_sd = "paren")),
                   "Removal" = list("Yes" = ~ n_perc0(.data$CatheterStatus != "censor"),
                                    "No" = ~ n_perc0(.data$CatheterStatus == "censor")),
                   "Mean Removal Time" = list("Mean (SD)" = ~ mean_sd(.data$RemovalDur, na_rm = T, show_n = "never", denote_sd = "paren")))
...
ggplot(removaldurdt, aes(x=RemovalDur, y=cumsum(n))) +
  geom_point() + geom_line() +
  scale_x_continuous(limits = c(0,364), name = "Duration of Complication (day)") +
  scale_y_continuous(limits = c(0, 110), name = "No. of Patient (Cumulative)")
