...
pdcds0 %>% mutate_if(is.labelled, ~(as_factor(.))) %>% select(-c(DataID, Comorbidities)) %>% datatable(fillContainer = T)
```
...
pdcds1 %>% select_if(is.factor) %>% select(-CxTypeCode) %>% summary() %>% kable()
pdcds1 %>% select(CxTypeCode) %>% summary() %>% kable()
```
