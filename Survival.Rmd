...
```{r}
ggsurvplot(compx_km, data = pdcds1, risk.table = T, pval = T)
```

KM Plot for catheter removal was shown below

```{r}
ggsurvplot(removal_km, data = pdcds1, risk.table = T, pval = T)
```
...
when adjusted to gender, age and diabetes status, there was no significant association between method and time-to-event of catheter removal, with time ratio (TR) of peritonesocopy method (as compare to percutaneous method) to catheter removal was `r round(removalsurvweimod_res$ETR, 2)[4,1]` (95% CI = `r round(removalsurvweimod_res$ETR, 2)[4,2:3]`). 

the hazard ratio (HR) of peritonesocopy method (as compare to percutaneous method) to catheter removal was `r round(removalsurvweimod_res$HR, 2)[4,1]` (95% CI = `r round(removalsurvweimod_res$HR, 2)[4,2:3]`) times higher, when adjusted to gender, age and diabetes status.
